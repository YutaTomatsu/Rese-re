...
<body class="body">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var previousSort = localStorage.getItem('sort');
            if (previousSort) {
                document.getElementById('sort').value = previousSort;
            }
            sortShops();
        });
        ...
        function sortShops() {
            ...
            var shops = document.querySelectorAll('.card');
            var sortedShops = Array.from(shops).sort(function(a, b) {
                var aValue = parseFloat(a.querySelector('.shop-reviewsAvg').textContent);
                var bValue = parseFloat(b.querySelector('.shop-reviewsAvg').textContent);

                if (isNaN(aValue)) aValue = sortValue === 'high' ? -Infinity : Infinity;
                if (isNaN(bValue)) bValue = sortValue === 'high' ? -Infinity : Infinity;
            ...
            var parentDiv = document.querySelector('.card__row');
            parentDiv.innerHTML = '';
            sortedShops.forEach(function(shop) {
                parentDiv.appendChild(shop);
            });
        }
    </script>
