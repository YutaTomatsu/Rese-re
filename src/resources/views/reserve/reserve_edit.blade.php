                        ...
                        <div class="item">
                            <div class="column">
                                ...
                                <select class="text" name="time" id="time-select" value="{{ $reservation->time }}">
                                    ...
                                    @for ($j = 0; $j < 60; $j +=15) @php $time=str_pad($i, 2, '0' , STR_PAD_LEFT) . ':' . str_pad($j, 2, '0' , STR_PAD_LEFT); @endphp <option value="{{ $time }}" {{ old('time', date('H:i', strtotime($reservation->time))) == $time ? 'selected' : '' }} {{ ($is_today && ($i > 21 || ($i === 21 && $j > 00))) || $i < 17 ? 'disabled' : '' }}>
                                        {{ $time }}
                                        </option>
                                        ...
                                        return hour >= 17 && (hour < 21 || (hour===21 && minute <=00)); }); @endfor @endforeach </select>
                                            <select class="text" name="number_of_people" value="{{ $reservation->number_of_people }}">
                                                @for ($i = 1; $i <= 20; $i++) <option value="{{ $i }}" {{ old('number_of_people', $reservation->number_of_people) == $i ? 'selected' : '' }}>
                                                    {{ $i }}人</option>
                                                    @endfor
                                            </select>
