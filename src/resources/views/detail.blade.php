    ...
    <div class="content__box">
        <div class="between">
            <div class="detail__left">
                ...
                <div class="review__center" id="item2">
                    <div class="review">

                        <button class="review__all">全ての口コミ情報</button>

                        <form class="review__write" action="{{ route('review') }}" method="get">
                            @csrf
                            <input type="hidden" name="shop_id" value="{{ request()->query('id') }}">
                            <button class="review__button" type="submit">口コミを投稿する</button>
                        </form>
                        ...
                        <div class="review__content">
                            ...
                            <div class="review__item" style="display: none;" data-user="{{ (Auth::id() === $review->user_id) ? 'true' : 'false' }}">
                                @if (Auth::id() === $review->user_id)
                                <div class="edit__delete__box">
                                    <a href="{{route('review.edit',['id'=>$review->id])}}" class="review-edit">クチコミを編集</a>
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                ...
                                var button = document.querySelector('.review__all');
                        ...
                        <div class="item">
                            <div class="column">
                                ...
                                <select class="text" name="time" id="time-select">
                                    ...
                                    @for ($j = 0; $j < 60; $j +=15) @php $time=str_pad($i, 2, '0' , STR_PAD_LEFT) . ':' . str_pad($j, 2, '0' , STR_PAD_LEFT); @endphp <option value="{{ $time }}" {{ old('time') == $time ? 'selected' : '' }} {{ ($is_today && ($i > 21 || ($i === 21 && $j > 00))) || $i < 17 ? 'disabled' : '' }}>
                                        {{ $time }}
                                        </option>
                                        ...
                                        return hour >= 17 && (hour < 21 || (hour===21 && minute <=00)); }); @endfor @endforeach </select>
                                            <select class="text" name="number_of_people">
                                                @for ($i = 1; $i <= 20; $i++) <option value="{{ $i }}" {{ old('number_of_people') == $i ? 'selected' : '' }}>{{ $i }}人
                                                    </option>
                                                    @endfor
                                            </select>
