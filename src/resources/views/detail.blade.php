...
document.querySelector('.date').addEventListener('change', function() {
    ...
    if (isToday) {
        ...
        timeRange = Array.from({ length: 24 * 4 - currentHour * 4 - parseInt(new Date().getMinutes() / 15) - 1 }, (_, i) => {
            const hours = currentHour * 4;
            const minutes = parseInt(new Date().getMinutes() / 15) + 1;
            return i + hours + minutes;
        });
    } else {
        timeRange = Array.from({ length: 24 * 4 }, (_, i) => i);
    }
    ...
    timeRange.forEach(time => {
        ...
        if (selectedDate.toDateString() === new Date().toDateString() && option.value < new Date().toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})) {
